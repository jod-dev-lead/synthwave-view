...
const validateEnv = () => {
  ...
  if (!import.meta.env.OPENROUTER_API_KEY) {
    ...
    console.error('4. Add OPENROUTER_API_KEY with your OpenRouter API key');
  ...
  if (import.meta.env.OPENROUTER_API_KEY === 'your_openrouter_api_key_here') {
  ...
  if (!import.meta.env.OPENROUTER_API_KEY.startsWith('sk-or-v1-')) {
    console.error('❌ OPENROUTER_API_KEY format is incorrect!');
    console.error('OpenRouter API keys should start with "sk-or-v1-"');
    console.error('Your key starts with:', import.meta.env.OPENROUTER_API_KEY.substring(0, 10) + '...');
  ...
  static async sendMessage(messages: ChatMessage[]): Promise<ChatResponse> {
    try {
      ...
      if (error?.status === 401) {
        ...
        console.error('🔑 API Key Issue: Make sure OPENROUTER_API_KEY is set correctly in Vercel');
